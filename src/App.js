  ...
  return (
    <div className="min-h-screen bg-slate-50 p-2 md:p-8 font-sans text-slate-800">
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
        ...
        {/* Header */}
        <div className="bg-white px-4 md:px-6 py-6 md:py-8 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-center md:text-left">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 flex items-center justify-center md:justify-start gap-3">
              <Utensils className="w-6 h-6 md:w-8 md:h-8 text-orange-500 shrink-0" />
              우리 아이 튼튼 월간 식단표
            </h1>
            <p className="text-slate-500 mt-2 text-xs md:text-base break-keep">전문 영양사가 설계한 성장 3박자, 그리고 로켓배송 장보기까지 한 번에!</p>
          </div>
          
          <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-xl border border-slate-200 w-full md:w-auto justify-center">
          ...
          <button 
            ...
            className="hidden md:flex bg-slate-800 hover:bg-slate-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-colors items-center gap-2 shadow-md shrink-0">
            인쇄하기
          </button>
        </div>

        {/* Mobile Swipe Hint */}
        <div className="md:hidden flex items-center justify-center gap-2 py-3 bg-blue-50 text-blue-600 text-xs font-semibold border-b border-blue-100">
          <ChevronLeft className="w-4 h-4 animate-pulse" />
          <span>표를 좌우로 스와이프하여 식단표를 확인하세요</span>
          <ChevronRight className="w-4 h-4 animate-pulse" />
        </div>

        {/* Calendar Grid Wrapper for Mobile Horizontal Scroll */}
        <div className="w-full overflow-x-auto custom-scrollbar bg-slate-50 md:bg-white">
          <div className="p-4 md:p-6 min-w-[800px]">
            {/* Days Header */}
            <div className="grid grid-cols-7 gap-3 md:gap-4 mb-4">
              {['일', '월', '화', '수', '목', '금', '토'].map((day, i) => (
                <div key={day} className={`text-center font-bold text-sm py-2 rounded-lg bg-white border border-slate-100 shadow-sm md:bg-slate-50 md:border-none md:shadow-none ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-slate-600'}`}>
                  {day}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-3 md:gap-4">
              {blanks.map(b => <div key={`blank-${b}`} className="min-h-[140px] bg-slate-50/50 rounded-xl border border-slate-100/50" />)}
              {monthDays.map(day => {
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-6">
              <span className="px-3 md:px-4 py-1.5 bg-orange-100 text-orange-700 text-xs md:text-sm font-bold rounded-full border border-orange-200">
                {selectedMeal.date} {getTypeLabel(selectedMeal.type)}
              </span>
              {selectedMeal.isSeasonal && (
                <span className="px-3 md:px-4 py-1.5 bg-green-50 text-green-700 text-xs md:text-sm font-bold rounded-full flex items-center gap-1.5 border border-green-200">
                  🌱 제철 식재료 포함
                </span>
              )}
              {selectedMeal.isCustom && (
                <span className="px-3 md:px-4 py-1.5 bg-purple-50 text-purple-700 text-xs md:text-sm font-bold rounded-full flex items-center gap-1.5 border border-purple-200">
                  👨‍🍳 아빠표 특식
                </span>
              )}
            </div>

            <h2 className="text-xl md:text-3xl font-extrabold text-slate-800 mb-8 leading-tight">{selectedMeal.name}</h2>
            
            <div className="grid md:grid-cols-2 gap-8">
              {/* Ingredients List */}
              <div className="bg-slate-50 rounded-2xl p-4 md:p-6 border border-slate-100">
                <h3 className="font-bold text-base md:text-lg mb-4 flex items-center gap-2 text-slate-800">
                  🛒 필요 식재료
                </h3>
                  ...
                  {selectedMeal.ingredients.map((item, idx) => {
                    ...
                    return (
                      <li key={idx} className="flex flex-col gap-2">
                        ...
                        {altInfo && (
                          <div className="ml-2 md:ml-4 p-3 bg-orange-50 rounded-xl border border-orange-100 flex gap-2">
                            <span className="text-orange-500 mt-0.5">💡</span>
                            <div className="text-xs md:text-sm">
                              <span className="font-bold text-orange-800">대체 추천: {altInfo.alt}</span>
                              <p className="text-orange-600/80 mt-0.5 leading-snug break-keep">{altInfo.reason}</p>
                            </div>
                          </div>
                        )}
                      </li>
                ...
                <div className="mt-5 text-[11px] md:text-xs text-slate-500 flex items-start gap-2 bg-white p-3 rounded-xl border border-slate-200">
                  <Info className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                  <p className="break-keep">재료를 클릭하면 쿠팡(Coupang) 검색 결과(새 창)로 바로 이동해요! 빠른 로켓배송으로 준비해 보세요.</p>
                </div>
              </div>
              ...
              <div>
                ...
                {selectedMeal.videoId ? (
                      ...
                      <a href={`https://www.youtube.com/watch?v=${selectedMeal.videoId}`} target="_blank" rel="noopener noreferrer" className="block w-full h-full relative group">
                         ...
                         <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                           <div className="w-12 h-12 md:w-16 md:h-16 bg-red-600 rounded-full flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
                             <div className="w-0 h-0 border-y-[8px] border-y-transparent border-l-[12px] md:border-y-[10px] md:border-l-[16px] border-l-white ml-1"></div>
                           </div>
                         </div>
                      </a>
                    </div>
                    <p className="text-center text-[11px] md:text-xs text-slate-500 mt-3 font-medium break-keep">썸네일을 클릭하면 유튜브 원본 영상이 재생됩니다.</p>
                  </div>
                ) : (
                  <div className="aspect-video bg-slate-50 rounded-2xl border border-slate-200 border-dashed flex flex-col items-center justify-center text-slate-400">
                    <Utensils className="w-6 h-6 md:w-8 md:h-8 mb-2 opacity-50" />
                    <p className="text-xs md:text-sm">준비된 영상이 없습니다</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* Scrollbar and Animation Custom Styles */}
